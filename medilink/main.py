...
class Patient(BaseModel):
    ...
    gender:Annotated[Literal["male","female","others"],Field(...,description="City of the patient")]
...
def sort_patients(sort_by: str= Query(...,description="Sort on basis of height,weight or bmi"), order:str = Query('asc',description="sort in asc or desc order")):
    ...
    if order not in ['asc','desc']:
        raise HTTPException(status_code=400, detail="Invalid order select between asc and sesc")
